...
$stmtFolder = $pdo->prepare("SELECT d.folder_id FROM documents d JOIN document_annotations a ON a.document_id = d.id WHERE a.id = :id");
...
try {
    $stmt = $pdo->prepare("DELETE FROM document_annotations WHERE id = :id");
    $stmt->execute(['id' => $id]);
